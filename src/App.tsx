    ...
    setTimeout(() => {
      setCalendarDays((days) => {
        ...
        const newRecord: BookingRecord = {
          ...
          chain,
          // Auto-approved on submit: there is no manual review step, so the
          // record must not sit in PENDING while the success screen tells the
          // customer their booking is confirmed.
          status: "APPROVED",
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-5 h-16 flex items-center justify-between gap-3 sm:gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <a
              ...
              className="font-sans text-base font-semibold h-10 w-10 justify-center sm:h-auto sm:w-auto sm:justify-start px-0 sm:px-3 sm:py-2 rounded-xl bg-[#F7EBDA]/80 sm:bg-transparent text-[#2C2820] hover:bg-[#2C2820]/10 active:scale-95 transition-all duration-200 flex items-center gap-2 flex-shrink-0"
            >
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4 sm:w-4 sm:h-4">
      ...
      <main className={cn("flex-1 flex flex-col", isFullBleed ? "" : "max-w-6xl w-full mx-auto px-4 sm:px-6 py-6 sm:py-10 md:pb-14 pt-20 md:pt-[115px]")}>
        ...
        {step === "existing" && booking.customer && booking.bookingId && (
          <div className="animate-[fadeIn_0.4s_ease]">
            <ExistingBooking
              booking={booking}
              status={bookingRecords.find((r) => r.id === booking.bookingId)?.status ?? "APPROVED"}
